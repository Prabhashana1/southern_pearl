...
class user
{
   ...
   FROM rooms WHERE (checkin <= '$checkin' AND checkout >='$checkout') OR (checkin >= '$checkin' AND checkin <='$checkout') OR (checkin <= '$checkin' AND checkout >='$checkin') )";
        $check = mysqli_query($this->db, $sql)  or die(mysqli_connect_errno() . "Query Doesn't run");;
    ...
    public function booknow($checkin, $checkout, $email, $phone, $roomname)
    {
        ...
        if (mysqli_num_rows($check) > 0) {
            ...
            $sql2 = "UPDATE rooms  SET checkin='$checkin', checkout='$checkout', email='$email', phone='$phone', book='true' WHERE room_id=$id";
            $sql3 = "UPDATE room_category SET available = available -1 WHERE roomname = '$roomname'";
            $sql4 = "UPDATE room_category SET booked = booked +1 WHERE roomname = '$roomname'";
    ...
    public function edit_all_room($checkin, $checkout, $email, $phone, $id)
    {
